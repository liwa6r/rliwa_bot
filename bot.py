...
# ══════════════════════════════════════════════════
async def _flush_media_group_job(context):
    """job_queue callback — يُشغَّل بعد 3 ثواني من آخر صورة"""
...
async def receive_photo(update, context):
    ...
    if media_group_id:
        ...
        # احذف الـ job القديم وأنشئ جديد (sliding window 3 ثوانٍ)
        job_name = f"flush_{user_id}"
