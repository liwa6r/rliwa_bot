...
# ══════════════════════════════════════════════════
async def _flush_media_group(user_id: int, context, update):
    """ينتظر 1.5 ثانية ثم يطلب الموقع لكل الصور المجمّعة"""
    ...
    session["pending_photos"] = list(group)
    session["pending_group"]  = []
    session["group_timer"]    = None
    # نأخذ أول صورة كـ pending_photo
    session["pending_photo"] = session["pending_photos"].pop(0)
    count = len(session["pending_photos"]) + 1  # كل الصور
...
async def receive_photo(update, context):
    ...
    # تحقق: إذا كانت pending_photo موجودة ولم تُضاف بعد، أضفها بدون بيانات
    # (يحمي من فقدان الصور عند الإرسال المتتالي)
    ...
    if media_group_id:
        ...
        if old_timer and not old_timer.done():
            old_timer.cancel()
        # إنشاء مؤقت جديد 1.5 ثانية
        loop = asyncio.get_event_loop()
        timer = loop.create_task(_flush_media_group(user_id, context, update))
