...
def new_session(user_id, name, dept):
    user_sessions[user_id] = {
        ...
        "last_coords_dms":  "",
        "pending_plot":     "",
        "collecting":       False,   # وضع جمع الصور (بعد الموقع)
        "collected_photos": [],      # الصور المجمّعة بعد الموقع
    }
...
async def receive_name(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if dept == "inspection":
        ...
        await update.message.reply_text(
            ...
            "2️⃣ أو أرسل رابط جوجل ماب\n"
            "3️⃣ أو أرسل إحداثيات: `23.085, 54.016`\n\n"
...
async def receive_photo(update, context):
    ...
    session["queued_photos"] = session.get("queued_photos", [])
    session["queued_photos"].append(photo_bytes)
    if not session.get("last_location"):
        await update.message.reply_text(
...
async def process_location(update, session, lat, lng):
    ...
    await update.message.reply_text(
        ...
        "عندما تنتهي اضغط *انتهيت من الصور* ✅",
        parse_mode="Markdown",
        reply_markup=ReplyKeyboardMarkup(
            [[KeyboardButton("✅ انتهيت من الصور")]],
            resize_keyboard=True
...
async def next_after_location(session, update=None):
    """بعد الانتهاء من الصور — توعية تسأل عن النوع، تفتيش تسأل عن الملاحظة"""
    if session.get("dept") == "awareness":
        if update:
            await update.message.reply_text(
                "📋 ما نوع التوعية؟",
...
async def finish_collecting(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """معالجة الضغط على انتهيت من الصور في وضع COLLECTING_PHOTOS"""
    user_id = update.effective_user.id
    session = user_sessions.get(user_id, {})
    text    = update.message.text.strip()
    ...
    # إذا وصلت صورة إضافية في هذه المرحلة
    ...
    if "انتهيت" in text:
        ...
        if not photos:
            await update.message.reply_text("❌ لم ترسل أي صورة! أرسل صورة أولاً.")
            return COLLECTING_PHOTOS
        ...
        await update.message.reply_text(
            f"✅ تم إضافة *{len(photos)}* صورة\n"
            f"📊 إجمالي الصور: *{total}*",
            parse_mode="Markdown",
            reply_markup=ReplyKeyboardRemove()
        )
        return await next_after_location(session, update)

    # نص غير معروف — نفس الموقع أو موقع جديد
    if "موقع جديد" in text:
        session["collecting"] = False
        session["collected_photos"] = []
        await ask_location(update)
