...
async def run_system_check(update: Update):
    ...
    try:
        ...
        results.append(f"✅ Gmail — متصل ({GMAIL_USER})")
        gmail_ok = True
    except Exception as e:
        results.append(f"❌ Gmail — {str(e)[:50]}")
        all_ok = False
    ...
    if gmail_ok:
        try:
            ...
            msg["From"]    = GMAIL_USER
            msg["To"]      = ADMIN_EMAIL
            msg["Subject"] = f"🔧 فحص النظام — {BOT_NAME} {BOT_VERSION}"
            body = (
                f"تقرير فحص النظام\n\n"
...
async def receive_location_gps(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    return await next_after_location(session)
...
def create_pptx(session: dict) -> BytesIO:
    ...
    def photo_card(slide, photo_bytes, x, y, w, h, loc_name, note, awareness_type=""):
        ...
        if awareness_type: info_rows.append(("📋", awareness_type))
        ...
        try:
            slide.shapes.add_picture(
                BytesIO(photo_bytes), x+pad, y+pad, w-pad*2, img_h-pad
            )
        except Exception as e:
...
def send_email(buf: BytesIO, session: dict, fname: str) -> bool:
    ...
    try:
        ...
        msg["From"]    = GMAIL_USER
        msg["To"]      = ADMIN_EMAIL
        msg["Subject"] = f"تقرير {dept_ar} — {session.get('name','')} — {session.get('date','')}"
