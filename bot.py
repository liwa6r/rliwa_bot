...
async def _flush_media_group(user_id: int, bot, chat_id: int):
    """ينتظر 3 ثوانٍ من آخر صورة ثم يطلب الموقع"""
    ...
    # تحقق إن هذا الـ task هو الحالي (لم يُلغَ)
    if session.get("group_timer") is not asyncio.current_task():
        return
...
async def receive_photo(update, context):
    ...
    if media_group_id:
        ...
        # إلغاء الـ task القديم وإنشاء جديد (sliding window 3 ثوانٍ)
        old_task = session.get("group_timer")
