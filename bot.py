...
(WAITING_CODE, WAITING_NAME, MAIN_MENU,
 ...
 CONFIRM_SEND, COLLECTING_PHOTOS) = range(15)
...
def create_pptx(session: dict) -> BytesIO:
    ...
    def draw_ba_cell(slide, entry, cx, cy, cw, ch, seq_num):
        """
        يرسم خلية واحدة قبل/بعد داخل الصفحة:
        - النصف الأيمن: صورة القبل
        - النصف الأيسر: مربع فارغ للبعد
        - شريط البيانات أسفل الخلية
        - رقم تسلسلي في زاوية الخلية
        """
        p_gap     = Inches(0.1)   # مسافة بين القبل والبعد داخل الخلية
        DATA_H    = Inches(0.78)  # ارتفاع ثابت لشريط البيانات
        img_area_h = ch - DATA_H - Inches(0.04)
        half_w     = (cw - p_gap) / 2

        # ── رقم تسلسلي (زاوية يمين عليا)
        num_box = slide.shapes.add_shape(1, cx, cy, Inches(0.32), Inches(0.32))
        num_box.fill.solid(); num_box.fill.fore_color.rgb = header_color
        num_box.line.fill.background()
        txt(slide, str(seq_num), cx, cy, Inches(0.32), Inches(0.32),
        ...
        # ── تسمية "قبل" صغيرة داخل الإطار
        txt(slide, "قبل", bx + Inches(0.05), cy + Inches(0.04),
            half_w - Inches(0.1), Inches(0.2),
            size=7, bold=True, color=header_color, align=PP_ALIGN.RIGHT)
        ...
        txt(slide, "بعد", ax + Inches(0.05), cy + Inches(0.04),
            half_w - Inches(0.1), Inches(0.2),
            size=7, bold=True, color=RGBColor(0xBB, 0xBB, 0xBB), align=PP_ALIGN.LEFT)

        txt(slide, "أضف صورة البعد هنا",
            ax, cy + img_area_h / 2 - Inches(0.18), half_w, Inches(0.36),
            size=8.5, italic=True, color=RGBColor(0xCC, 0xCC, 0xCC), align=PP_ALIGN.CENTER)
        ...
        ROW = Inches(0.36)
        for ri, (lb, vl) in enumerate(rows[:2]):   # حد أقصى سطرين في الشريط
            ry = dy + Inches(0.04) + ri * ROW
            txt(slide, lb,
                cx + Inches(0.08), ry, cw - Inches(0.16), Inches(0.16),
                size=6.5, bold=True, color=header_color, align=PP_ALIGN.RIGHT)
            txt(slide, vl,
                cx + Inches(0.08), ry + Inches(0.16), cw - Inches(0.16), Inches(0.2),
