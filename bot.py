...
async def finish_collecting(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    session["collected_photos"]  = []
    session["collecting_msg_id"] = None
    session["last_batch_count"]  = count_added  # لاستخدامه في receive_awareness_type
...
def create_pptx(session: dict) -> BytesIO:
    ...
    def make_ba_slide(entry, idx):
        ...
        txt(slide, f"قبل وبعد — موقع {idx}",
            Inches(0.5), Inches(0.2), Inches(6), Inches(0.3),
            size=9, bold=True, color=WHITE, align=PP_ALIGN.RIGHT)
        txt(slide, f"{date}  {now_time}",
            W-Inches(4.5), Inches(0.2), Inches(4), Inches(0.3),
            size=9, color=WHITE, align=PP_ALIGN.LEFT)

        pad = Inches(0.22); gap = Inches(0.2)
        bw  = (W - pad*2 - gap) / 2
        bh  = H - Inches(0.75)
        my  = Inches(0.55)

        txt(slide, "◄ قبل", pad, my - Inches(0.28), bw, Inches(0.25),
            size=10, bold=True, color=header_color, align=PP_ALIGN.RIGHT)
        photo_card(slide, entry["photo"], pad, my, bw, bh,
                   {"location": entry["location"], "note": "",
                    "coords_dms": entry.get("coords_dms",""),
                    "plot_no": "", "awareness_type": ""})

        txt(slide, "بعد ►", pad+bw+gap, my - Inches(0.28), bw, Inches(0.25),
            size=10, bold=True, color=header_color, align=PP_ALIGN.LEFT)
        photo_card(slide, entry["photo_after"], pad+bw+gap, my, bw, bh,
                   {"location": entry["location"], "note": entry.get("note",""),
                    "coords_dms": entry.get("coords_dms",""),
                    "plot_no": entry.get("plot_no",""), "awareness_type": ""})
